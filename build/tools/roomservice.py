...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from RyzenOS Github (http://github.com/RyzenOS-Devices)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:RyzenOS-Devices+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the Ryzen snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/ryzen.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('RyzenOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: RyzenOS/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "RyzenOS/%s" % repo_name,
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/ryzen.dependencies'
    syncable_repos = []
    verify_repos = []
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/RyzenOS/" + repo_name + "/branches")
    add_auth(githubreq)
    result = json.loads(urllib.request.urlopen(githubreq).read().decode())
...
print("Repository for %s not found in the RyzenOS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
