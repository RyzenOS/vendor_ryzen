...
// Copyright (C) 2018,2021 The RyzenOS Project
//
...
func init() {
	android.RegisterModuleType("ryzen_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ryzenExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ryzenExpandVariables(ctx, dep_file)
	...
	cmd := ryzenExpandVariables(ctx, String(g.properties.Cmd))
